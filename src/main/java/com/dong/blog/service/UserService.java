...
public class UserService {
	...
	public void 회원수정(User  requestUser) {
		...
		if(!user.getRole().equals(RoleType.KAKAO_OAUTHUSER)) {
	...
	/*
	 * @Transactional public OAuthToken kakao토큰받기(String code) {
	 * 
	 * 
	 * RestTemplate restTemplate =new RestTemplate();
	 * 
	 * // http 해더 오브젝트 생성 HttpHeaders headers = new HttpHeaders();
	 * headers.add("Content-type","application/x-www-form-urlencoded;charset=utf-8")
	 * ;
	 * 
	 * // http 바디 오브젝트 생성 MultiValueMap<String, String> params = new
	 ...
	 * 
	 * // http 해더와 바디를 하나의 오브젝트에 담기 //ResponseEntity::exchange함수가 HttpEntity를 받기 때문에
	 * 하나의 오브젝트에 담는다 HttpEntity<MultiValueMap<String, String>> kakaoTokenRequst =
	 ...
	 * OAuthToken.class); } catch (JsonMappingException e) { // TODO Auto-generated
	 * catch block e.printStackTrace(); } catch (JsonProcessingException e) { //
	 ...
	 * }
	 * 
	 * @Transactional public KakaoProfile user정보받기(OAuthToken oauthToken) {
	 * 
	 * 
	 * 
	 * RestTemplate restTemplate =new RestTemplate(); // http 해더 오브젝트 생성 HttpHeaders
	 ...
	 * 
	 * // http 해더와 바디를 하나의 오브젝트에 담기 //ResponseEntity::exchange함수가 HttpEntity를 받기 때문에
	 * 하나의 오브젝트에 담는다 HttpEntity<MultiValueMap<String, String>> kakaoProfileRequest =
	 * new HttpEntity<>(headers);
	 * 
	 * //Http 요청하기 - 토큰 반환 받을 주소, post 방식, 그리고 해더와 바디가 포함된 오브젝트, 반환 받을 종류
	 * ResponseEntity<String> response =
	 * restTemplate.exchange("https://kapi.kakao.com/v2/user/me" ,HttpMethod.POST
	 * ,kakaoProfileRequest ,String.class);
	 * 
	 * ObjectMapper objectMapper = new ObjectMapper(); KakaoProfile kakaoProfile =
	 * null; try { kakaoProfile = objectMapper.readValue( response.getBody(),
	 * KakaoProfile.class); } catch (JsonMappingException e) { // TODO
	 * Auto-generated catch block e.printStackTrace(); } catch
	 * (JsonProcessingException e) { // TODO Auto-generated catch block
	 * e.printStackTrace(); } return kakaoProfile;
	 * 
	 * }
	 * 
	 * @Transactional public void kakao회원처리(KakaoProfile kakaoProfile) {
	 * System.out.println( );
	 * System.out.println(kakaoProfile.getKakao_account().getEmail());
	 * System.out.println(defaultPasswordKey); User kakaoUser = User .builder()
	 ...
	 * .role(RoleType.KAKAO_OAUTHUSER) .build(); User originUser = 회원찾기(kakaoUser);
	 * if(originUser==null)회원가입(kakaoUser); 회원세션변경(kakaoUser);
	 * 
	 * System.out.println("***************************************************22222"
	 * );
	 * 
	 * // 회원세션처리 회원세션변경(kakaoUser); }
	 */
